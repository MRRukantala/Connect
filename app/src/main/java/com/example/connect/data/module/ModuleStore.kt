...
class ModuleStore {
    @Singleton
    @Provides
    fun provideApi(retrofit: Retrofit): ProductApiClient {
        return retrofit.create(ProductApiClient::class.java)
    }

    @Singleton
    @Provides
    fun provideRepository(apiClient: ProductApiClient): ProductApiRepository {
        return StoreRepositoryInteractor(apiClient)
    }
}